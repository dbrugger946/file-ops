...
logging.basicConfig(filename="../logs/conversion.log",
...
def convert_file_to_pdf(file_path, output_dir):
    """ This function calls libreoffice directly in the macOS env
    ...
    try:
        ...
        if not os.path.exists(libreoffice_path):
            ...
            logging.error("Please install LibreOffice from https://www.libreoffice.org/download/download/")
            return None

        # Build command as list for better subprocess handling on macOS
        cmd = [
            libreoffice_path,
            '--headless',
            '--convert-to', 'pdf',
        ...
        logging.debug(f"Command executed successfully: {' '.join(cmd)}")
        ...
        logging.error("Command not found. Check if LibreOffice is installed at /Applications/LibreOffice.app")
